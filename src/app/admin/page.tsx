...
export default function AdminPage() {
  ...
  if (error) return <div style={{ color: 'var(--rose)', padding: '2rem', textAlign: 'center' }}>Error: {error}</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '1.25rem' : '2rem' }}>
      ...
      <div className="admin-box">
        ...
        <div style={{ 
          ...
          <AdminAction icon="➕" label="Promo" sub="WIP / Marketing" isMobile={isMobile} />
          <AdminAction icon="📢" label="Broadcast" sub="All users" isMobile={isMobile} />
          <AdminAction icon="🧹" label="Cleanup" sub="Redis Refresh" isMobile={isMobile} />
          <AdminAction icon="🛡️" label="Audit" sub="System events" isMobile={isMobile} />
        </div>
      </div>
...
function AdminAction({ icon, label, sub, isMobile }: any) {
  return (
    <div style={{
